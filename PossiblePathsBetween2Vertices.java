...
class Solution {
    ...
    public int countPaths(int V, ArrayList<ArrayList<Integer>> adj, int source,
        ...
        // Code here
        int count=0;
        count=dfs(V,adj,source,destination,count);
        
        return count;
    }
    
    private int dfs(int V,ArrayList<ArrayList<Integer>> adj,int s,int d,int count){
        ...
        if(s==d){
          ...
          for(int i=0;i<adj.get(s).size();i++){
            count+=  dfs(V,adj,adj.get(s).get(i),d,count);
          }
